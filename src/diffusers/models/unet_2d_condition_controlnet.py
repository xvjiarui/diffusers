...
@dataclass
class ControlUNetOutput(BaseOutput):
    """
    Args:
    ...
    down_block_res_samples: torch.FloatTensor
    zero_conv_sample: torch.FloatTensor
    zero_conv_down_block_res_samples: torch.FloatTensor
...
class ControlledUNet2DConditionModel(UNet2DConditionModel):
    def forward(
        ...
        control_samples: List[torch.Tensor],
        ...
        if self.mid_block is not None:
            ...
            # Control
            sample += control_samples[-1]
            control_samples = control_samples[:-1]

        assert len(control_samples) == len(
            down_block_res_samples
        ), f"{len(control_samples)} != {len(down_block_res_samples)}"
        # 5. up
        for i, upsample_block in enumerate(self.up_blocks):
            is_final_block = i == len(self.up_blocks) - 1

            res_samples = down_block_res_samples[-len(upsample_block.resnets) :]
            control_res_samples = control_samples[-len(upsample_block.resnets) :]

            res_samples = [res + control for res, control in zip(res_samples, control_res_samples)]

            down_block_res_samples = down_block_res_samples[: -len(upsample_block.resnets)]
            control_samples = control_samples[: -len(upsample_block.resnets)]
...
class ControlNetModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def forward(
        ...
        # 3. down
        down_block_res_samples = (sample,)
        zero_conv_down_block_res_samples = (self.conv_in_zero_conv(sample),)
        for downsample_block in self.down_blocks:
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples, zero_conv_samples = downsample_block(
                    hidden_states=sample,
                    temb=emb,
                    encoder_hidden_states=encoder_hidden_states,
                    attention_mask=attention_mask,
                ...
                sample, res_samples, zero_conv_samples = downsample_block(hidden_states=sample, temb=emb)

            down_block_res_samples += res_samples
            zero_conv_down_block_res_samples += zero_conv_samples
        ...
        if not return_dict:
            return (
                sample,
                down_block_res_samples,
                self.middle_block_zero_conv(sample),
                zero_conv_down_block_res_samples,
            )

        return ControlUNetOutput(
            sample=sample,
            down_block_res_samples=down_block_res_samples,
            zero_conv_sample=self.middle_block_zero_conv(sample),
            zero_conv_down_block_res_samples=zero_conv_down_block_res_samples,
        )
