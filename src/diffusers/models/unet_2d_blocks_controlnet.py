...
class CrossAttnDownBlock2DWithZeroConv(nn.Module):
    def __init__(
        ...
        if add_downsample:
            ...
            self.downsamplers_zero_conv = nn.ModuleList([zero_conv(out_channels)])
        else:
    ...
    def forward(
        ...
        output_states = ()
        zero_conv_output_states = ()

        #########################
        # TODO: support zero_conv
        #########################

        for resnet, attn, zero_conv_block in zip(self.resnets, self.attentions, self.zero_convs):
            ...
            if guided_hint is not None:
                hidden_states += guided_hint
                guided_hint = None

            output_states += (hidden_states,)
            zero_conv_output_states += (zero_conv_block(hidden_states),)

        if self.downsamplers is not None:
            for downsampler, zero_conv_block in zip(self.downsamplers, self.downsamplers_zero_conv):
                hidden_states = downsampler(hidden_states)

                output_states += (hidden_states,)
                zero_conv_output_states += (zero_conv_block(hidden_states),)

        return hidden_states, output_states, zero_conv_output_states
...
class DownBlock2DWithZeroConv(nn.Module):
    ...
    def forward(self, hidden_states, temb=None):
        ...
        for resnet, zero_conv_block in zip(self.resnets, self.zero_convs):
            if self.training and self.gradient_checkpointing:
